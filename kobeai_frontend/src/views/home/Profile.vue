  ...
  <div class="home-container">
    <div class="user-profile">
      <div class="user-header">
        <el-avatar :size="64" :src="authStore.user?.avatar || '/default-avatar.png'" />
        <div class="user-info">
          <h2>{{ authStore.user?.username }}</h2>
          <p class="user-role">{{ formatRole(authStore.user?.userRole) }}</p>
        </div>
      </div>
      ...
      <div class="user-details">
        ...
        <div class="detail-item">
          ...
          <span>{{ authStore.user?.phone || '未设置手机' }}</span>
        </div>
        <div class="detail-item">
          <el-icon><Timer /></el-icon>
          <span>会员到期时间：{{ formatDate(authStore.user?.membershipEndTime) }}</span>
        </div>
        <div class="detail-item">
          <el-icon><Edit /></el-icon>
          <span>{{ authStore.user?.bio || '这个人很懒，什么都没写~' }}</span>
        </div>
      </div>
...
const formatRole = (role?: string) => {
  ...
  return role ? roleMap[role] || role : '普通用户'
}
